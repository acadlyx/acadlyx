...
export function DashboardShell({
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_top_right,_rgba(34,211,238,.12),_transparent_28%),linear-gradient(135deg,#f8fafc,#eef6ff)] text-slate-900">
      ...
      <div className="mx-auto flex max-w-[1500px]">
        <aside className={`${open ? "block" : "hidden"} fixed inset-y-16 left-0 z-30 w-64 overflow-y-auto border-r border-slate-200/80 bg-white/95 p-3 shadow-xl backdrop-blur lg:sticky lg:top-16 lg:block lg:h-[calc(100vh-4rem)] lg:self-start lg:shadow-none`}>
        ...
        <main className="min-w-0 flex-1 overflow-x-hidden p-4 sm:p-6 lg:p-8"><button aria-label="Open navigation" className="mb-4 rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-semibold shadow-sm lg:hidden" onClick={() => setOpen((value) => !value)}>☰ Menu</button>{children}</main>
      </div>
    </div>
  );
}
