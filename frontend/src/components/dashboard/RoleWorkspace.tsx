...
import { AuthRequiredError, authedFetch, getCurrentUser, isAuthenticated, logout, AuthUser } from "@/lib/auth";
...
export function RoleWorkspace({ title, roles }: { title: string; roles: string[] }) {
  ...
  const allowedRolesKey = roles.join(",");
  useEffect(() => { if (!isAuthenticated()) { router.replace("/login"); return; } Promise.all([getCurrentUser(), authedFetch<{ success: true; data: Workspace }>("/erp/me/workspace")]).then(([currentUser, workspace]) => { if (!currentUser.roles.some(r => roles.includes(r))) { router.replace("/login"); return; } setUser(currentUser); setData(workspace.data); }).catch((e: Error) => { if (e instanceof AuthRequiredError) router.replace("/login"); else setError(e.message); }); }, [router, allowedRolesKey]);
