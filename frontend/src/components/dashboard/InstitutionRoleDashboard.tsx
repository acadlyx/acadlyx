...
const config: Record<string, { title: string; subtitle: string; focus: string[] }> = {
  ...
  MANAGEMENT: { title: "Management Dashboard", subtitle: "Executive command center for AIMT", focus: ["Institution KPIs", "Growth & operations", "Academic health", "Placement outcomes", "Decision support"] },
...
export function InstitutionRoleDashboard({ role }: { role: keyof typeof config }) {
  ...
  return <DashboardShell title={c.title} subtitle={c.subtitle} allowedRoles={[role]}>
    <div className="mx-auto max-w-7xl"><div className="mb-6 flex flex-col justify-between gap-3 sm:flex-row sm:items-end"><div><p className="text-xs font-bold uppercase tracking-[.18em] text-slate-400">AIMT · ACADLYX</p><h1 className="mt-2 text-2xl font-bold tracking-tight">{c.title}</h1><p className="mt-1 text-sm text-slate-500">{c.subtitle}</p></div><div className="flex flex-wrap gap-2">{c.focus.map(x => <span key={x} className="rounded-full bg-white px-3 py-1.5 text-xs font-medium text-slate-600 ring-1 ring-slate-200">{x}</span>)}</div></div>
