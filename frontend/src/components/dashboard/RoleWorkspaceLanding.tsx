...
import { hasAnyPermission, normalizeRoles, type CanonicalRole } from "@/lib/authorization";
import { ROLE_NAVIGATION, WORKSPACE_META, getPrimaryRole } from "./dashboardNavigation";
...
export function RoleWorkspaceLanding({ role }: { role: CanonicalRole }) {
  ...
  const currentRole = useMemo(() => {
    if (!user) return role;
    return getPrimaryRole(normalizeRoles(user.roles));
  }, [role, user]);
  ...
  const quickStart = available
    .filter((item) => item.href !== meta.home && item.label !== "Account security")
    .slice(0, 4);
        ...
        <section className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
          <div className="relative overflow-hidden bg-slate-950 px-6 py-8 text-white sm:px-8 lg:px-10 lg:py-10">
            ...
            <div className="relative max-w-3xl">
              ...
              <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-300 sm:text-base">
                This is your starting point. Only work relevant to your current responsibility is shown here.
              </p>
            </div>
          </div>
          ...
          <div className="grid gap-4 p-5 sm:p-6 lg:grid-cols-[1.2fr_.8fr]">
            ...
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-5">
              ...
              <p className="mt-2 text-xl font-black text-slate-900">{meta.label}</p>
              <p className="mt-2 text-sm leading-6 text-slate-500">
                Your navigation is automatically limited to the responsibilities and permissions assigned to your account.
              </p>
        ...
        <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
          <div className="mb-5">
            ...
            <p className="mt-1 text-sm text-slate-500">
              Use the categories below when you need something specific.
            </p>
          </div>
          ...
          <div className="space-y-6">
            {groups.map((group) => (
              <div key={group.label}>
                <h4 className="mb-3 text-sm font-black text-slate-800">{group.label}</h4>
